      ...
      commentService.addNewItem({ author: currentUser.id, content, postId: postId, parent: commentId }).then((res) => {
        ...
        setReplies({
          ...
          [commentId]: {
            ...
            comments: replies[commentId] ? [...replies[commentId]?.comments, replyComment] : [replyComment],
            exclude: replies[commentId] ? [...replies[commentId]?.exclude, replyComment?.id] : [replyComment?.id],
          },
        });
        setLoading(false);
        setContent("");
